...
const consumer = parseYaml(
  readFileSync(
    join(immeubleRoot, "contracts", "consumer_contract.yaml"),
    "utf8"
  )
);
...
const expectedSeedById = Object.fromEntries(
  seedLines
    .map((row) => {
...
const expectedCatalogIds = (catalog.projections ?? []).map((entry) => {
  ...
  const prefix =
    kind === "analysis_plan" ? "analysis_plan__" : "live_answer_view__";
...
if (listOk) {
  ...
  if (payload) {
    const artifacts = (payload.artifacts ?? payload.items ?? []).filter(
      Boolean
    );
...
for (const entry of catalog.projections ?? []) {
  ...
  if (!seed) {
    pushIssue(
      "seed_missing",
      `projection ${expectedId} missing in answer_artifacts.seed.jsonl`
    );
    continue;
  }
  ...
  if (artifactKind === "analysis_plan") {
    const questions =
      seedPayload.competency_questions ?? listedPayload?.competency_questions;
    if (!isArrayOrStringArray(questions)) {
      pushIssue(
        "payload",
        `${expectedId}: competency_questions missing or empty`
      );
    }
    ...
    if (
      !isArrayOrStringArray(
        seedPayload.refresh_checks ?? listedPayload?.refresh_checks
      )
    ) {
...
const missingFromRegistry = expectedLiveArtifactIds.filter(
  (id) => !listedIds.includes(id)
);
...
const audit = {
  ok:
    ...
    consumer.workspace_id === workspaceId &&
    listOk,
...
if (!listOk) {
  ...
  payloadIssues.push({
    label: "registry_list",
    detail:
      "artifact list command failed; cannot validate live answer registry state"
  });
}
...
if (
  catalog.projections &&
  catalogIds.length > 0 &&
  expectedLiveArtifactIds.length === 0
) {
  payloadIssues.push({
    label: "seed",
    detail: "no live_answer_view entries in answer_artifacts.seed.jsonl"
  });
}

audit.ok =
  payloadIssues.length === 0 && consumer.workspace_id === workspaceId && listOk;

writeFileSync(
  join(reportsDir, "projection_audit.json"),
...
writeFileSync(
  join(reportsDir, "consumer_contract.validation.json"),
  JSON.stringify(
    {
...
function parseArtifactPayload(artifact) {
  ...
  const raw =
    ...
    artifact.payload ??
    artifact.payload_raw ??
    artifact.payloadData;
