...
const CANONICAL_ONTOLOGY: BootstrapEntry[] = [
  ...
  },
  {
    schemaId: "mfo:ontology",
    content:
      "Know that GhostCrab must expose a stable public MCP surface while keeping mfo_* storage internals private.",
    facets: {
      criticality: "high",
      domain: "ghostcrab-product",
...
const PRODUCT_RECORDS: BootstrapEntry[] = [
  ...
  },
  {
    schemaId: "ghostcrab:runtime-component",
    content:
      "The PostgreSQL runtime stores mfo_* tables and powers facets, graph, projections, and migrations for GhostCrab.",
    facets: {
  ...
  },
  {
    schemaId: "ghostcrab:runtime-component",
    content:
      "The bootstrap seed populates mfo:system, mfo:schema, mfo:ontology, and first product records so the system can describe itself from cold start.",
    facets: {
  ...
  },
  {
    schemaId: "ghostcrab:roadmap-pr",
    content:
      "PR-2.2 delivered graph coverage, traversal, and learning tools on top of mfo_nodes and mfo_edges.",
    facets: {
  ...
  },
  {
    ...
    content:
      "Public branding and MCP API use ghostcrab_* names, while internal SQL tables and JSONB namespaces stay mfo_* and mfo:.",
    facets: {
...
export function getBootstrapSeedPlan(): BootstrapSeedPlan {
  return {
    ...
    summary:
      "Bootstrap data seeds mfo:system guidance, canonical schemas, starter ontologies, first factual GhostCrab product records, meta recipes for autonomy/projections across six activity families, a seeded product graph, and a ready-to-demo agent runtime state."
  };
}
...
export async function ensureBootstrapData(
  ...
  return database.transaction(async (queryable) => {
    ...
    for (const entry of ALL_BOOTSTRAP_ENTRIES) {
      ...
      if (entry.schemaId === "mfo:system") {
        summary.insertedSystemEntries += 1;
      } else if (entry.schemaId === "mfo:schema") {
        summary.insertedSchemas += 1;
      } else if (entry.schemaId === "mfo:ontology") {
...
async function ensureFacetEntry(
  ...
  const [existing] = await queryable.query<{
    ...
    `
      ...
      LIMIT 1
    `,
    [entry.schemaId, JSON.stringify(entry.lookupFacets)]
  );
  ...
  if (existing) {
    const existingFacets = JSON.stringify(existing.facets ?? {});
    ...
    if (
      ...
      await queryable.query(
        `
          ...
          WHERE id = $1
        `,
        [existing.id, serializedContent, serializedFacets]
      );
    }
...
async function ensureAgentState(
  ...
  await queryable.query(
    `
      INSERT INTO mfo_agent_state (agent_id, health, state, metrics)
...
async function ensureProjection(
  ...
  const [existing] = await queryable.query<{ id: string }>(
    `
      ...
      WHERE agent_id = $1
        AND scope IS NOT DISTINCT FROM $2::text
        AND proj_type = $3
        AND content = $4
      LIMIT 1
    `,
