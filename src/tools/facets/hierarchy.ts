...
export const hierarchyTool: ToolHandler = {
  definition: {
    ...
    description:
      "Return a hierarchical facet tree for the mfo_facets table using the pg_facets native extension. Requires pg_facets to be loaded.",
    inputSchema: {
      type: "object",
      properties: {
        top_n: {
  ...
  async handler(args, context) {
    ...
    const [tableOidRow] = await context.database.query<{ oid: string }>(
      `SELECT 'public.mfo_facets'::regclass::oid AS oid`
    );
    const tableOid = tableOidRow?.oid;
    if (!tableOid) {
      return createToolErrorResult(
        "ghostcrab_facet_tree",
        "Could not resolve OID for public.mfo_facets.",
        "internal_error"
      );
    }
