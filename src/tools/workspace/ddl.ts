...
import { triggerPreviewFromNative } from "../../db/native-trigger-preview.js";
import {
  inferBasicSemantics,
...
export const ddlProposeTool: ToolHandler = {
  ...
  async handler(args, context) {
    ...
    if (shouldTryOntologyValidation) {
      try {
        const validationRows = await context.database.query<{
          payload: {
            valid: boolean;
            warnings?: string[];
            generated_triggers?: string | null;
            conflicts?: Array<{
              type?: string;
              message?: string;
              pattern?: string;
        ...
        const validation = validationRows[0]?.payload;
        if (validation) {
          if (!validation.valid) {
            const firstConflict = validation.conflicts?.[0];
            const conflictMessage =
              firstConflict?.message ??
              firstConflict?.pattern ??
              firstConflict?.type ??
              "DDL proposal rejected by mb_ontology validation";

            return createToolErrorResult(
              "ghostcrab_ddl_propose",
              conflictMessage,
              firstConflict?.type === "workspace_not_found"
                ? "workspace_not_found"
                : "blocked_ddl_pattern",
              {
          ...
          if (validation.generated_triggers) {
            previewTrigger = validation.generated_triggers;
            triggerSummary = "Generated by mb_ontology.generate_triggers().";
          }
        }
      } catch (err) {
        console.debug(
          "[ghostcrab:ddl] mb_ontology.validate_ddl_proposal failed, falling back:",
