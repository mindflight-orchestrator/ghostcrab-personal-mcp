...
export interface TriggerGeneratorOptions {
  ...
  sourceTable: string;
  /**
   * Workspace id written to mfo_facets.workspace_id for every synced row.
  ...
  /**
   * Value written to mfo_facets.schema_id for scalar/array/ltree synced rows.
 ...
 * source table into Layer 2 (mfo_facets) based on a SyncFieldSpec array.
 *
 * ## Contract (migration 011 required)
 *
 * mfo_facets is expected to have:
 ...
 * - "array" → unnest() each element into a separate mfo_facets row,
 ...
 * - "geo" → INSERT/UPDATE into geo_entities (not mfo_facets).
 * - "embedding" → skipped.
 */
...
` : ""}
  ...
  -- INSERT / UPDATE: upsert fields into mfo_facets (or geo_entities for geo fields).
${functionBody}
...
/**
 * Merged scalar upsert: all scalar fields produce ONE mfo_facets row per source row.
 *
...
function buildMergedScalarInsert(
  ...
  INSERT INTO mfo_facets (source_ref, workspace_id, schema_id, facets, content)
  VALUES (
    NEW.${pkCol}::text,
    '${workspaceId}',
    ${schemaIdExpr},
  ...
  ON CONFLICT (source_ref, workspace_id) WHERE source_ref IS NOT NULL DO UPDATE SET
    facets    = mfo_facets.facets || ${facetsExpr},
    content   = ${contentExpr},
