...
/**
 * Register mfo_facets with pg_facets for native bitmap-accelerated search/count.
 *
 * Prerequisites:
...
export async function registerPgFacetsWithReport(
  ...
  try {
    ...
    const existingFacetRows = await database.query<{ facet_name: string }>(
      `SELECT facet_name
       FROM facets.list_table_facets_simple('public.mfo_facets'::regclass)`
    );
