...
/**
 * Resolves extension flags according to `MFO_NATIVE_EXTENSIONS`.
...
export async function resolveExtensionCapabilities(
  ...
  if (mode === "native") {
    if (!caps.pgFacets || !caps.pgDgraph || !caps.pgPragma) {
      throw new Error(
        `MFO_NATIVE_EXTENSIONS=native requires pg_facets, pg_dgraph, and pg_pragma. Found: pg_facets=${caps.pgFacets}, pg_dgraph=${caps.pgDgraph}, pg_pragma=${caps.pgPragma}`
      );
    }
  }
