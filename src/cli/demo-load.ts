...
function parseArgs(argv: string[]): {
  ...
  if (!profileId) {
    throw new Error("Usage: npm run demo:load -- --profile <profile-id>");
  }
...
async function main(): Promise<void> {
  const { profileId, skillsRepoRoot } = parseArgs(process.argv.slice(2));
  ...
  try {
    ...
    const summary = await database.transaction((queryable) =>
      loadDemoProfile(queryable, entries, profileId)
    );
