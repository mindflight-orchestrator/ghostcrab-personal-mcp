 ...
 * the Layer 2 surfaces (mfo_facets, graph.entity, graph.relation) have been
...
interface ExpectedCounts {
  ...
  layer2: {
    mfo_facets: { min: number; max: number };
...
interface DerivationReport {
  ...
  layer2: {
    mfo_facets: { count: number; ok: boolean };
...
async function verifyLayer2(
  ...
  const facetsResult = await client.query<{ count: string }>(
    `SELECT COUNT(*)::text AS count FROM mfo_facets WHERE workspace_id = $1`,
    [workspaceId]
  ).catch(() => ({ rows: [{ count: "0" }] }));
  ...
  return {
    mfo_facets: {
      count: facetCount,
      ok: facetCount >= expected.layer2.mfo_facets.min
    },
    graph_entity: {
      count: entityCount,
...
async function verifySearchability(
  ...
  for (const term of terms) {
    try {
      const result = await client.query<{ count: string }>(
        `SELECT COUNT(*)::text AS count FROM mfo_facets
...
async function seedFacetsFromLayer1(
  ...
  if (players.rows.length > 0) {
    for (const p of players.rows) {
      await client.query(
        `INSERT INTO mfo_facets (content, facets, schema_id, source_ref, workspace_id)
         VALUES ($1, $2, $3, $4, $5)
...
async function verify(): Promise<void> {
  ...
  console.log("\n── Layer 2 Verification ─────────────────────────────────────────");
  console.log(`  [${layer2.mfo_facets.ok ? "OK" : "FAIL"}] mfo_facets         ${layer2.mfo_facets.count} rows (min ${expected.layer2.mfo_facets.min})`);
