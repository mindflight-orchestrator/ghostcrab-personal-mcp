...
GhostCrab exposes ${manifest.total} registered MCP tools: ${manifest.basic} recommended default tools and ${manifest.extended} extended tools. MCP \`tools/list\` returns the recommended default set; \`gcp tools list\`, \`gcp tools verify\`, and \`ghostcrab_tool_search\` expose discovery metadata for the full catalog.
