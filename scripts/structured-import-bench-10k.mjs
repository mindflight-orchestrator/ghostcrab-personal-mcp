...
try {
  ...
  const applyRows =
    (apply.out.facets_inserted ?? 0) + (apply.out.facets_updated ?? 0);
  console.log(
    JSON.stringify({
      ok: true,
      rows_target: ROWS + 2,
      load_rows_per_sec: loadRows ? Math.round((loadRows / load.ms) * 1000) : 0,
      apply_rows_per_sec: applyRows
        ? Math.round((applyRows / apply.ms) * 1000)
        : 0,
      vendor_mindbrain: spawnSync(
        "git",
        [
          "-C",
          join(pkgRoot, "vendor", "mindbrain"),
          "rev-parse",
          "--short",
          "HEAD"
        ],
