...
**What “V1” means** (goals, definition of done, where plans live): **[docs/AUDIT_V1_TRACKING.md](docs/AUDIT_V1_TRACKING.md)** — read that first; this file does not redefine V1.
...
**Workspace + Layer1 coupling** (typed relational tables, `mindbrain` control plane, triggers into `mfo_facets` / optional graph-geo, `workspace_id` isolation): operational detail in **[docs/v3/RUNBOOK_V3.md](docs/v3/RUNBOOK_V3.md)** and architecture summary in **[README_ARCHITECTURE.md](README_ARCHITECTURE.md)** (sections *MCP tool surface (24 tools)* and *Workspace isolation and DDL lifecycle*).
...
**Operational commands:** `ghostcrab tools list` (JSON list of tool definitions); [`scripts/mcp-smoke.mjs`](scripts/mcp-smoke.mjs) validates the expected public tool families on startup. **Export contract:** [docs/contracts/workspace-model-export.schema.json](docs/contracts/workspace-model-export.schema.json). **Agent scenarios:** [docs/mcp_agent_scenarios.md](docs/mcp_agent_scenarios.md) — includes `workspace_create` and `workspace_ddl_propose` alongside facets/graph/pragma baselines.
...
- [x] **Native boot / seed contract** — root bootstrap path now uses the native PostgreSQL Docker image; `migrate` and server startup validate `pg_facets`, `pg_dgraph`, `pg_pragma`, plus native readiness before accepting `MFO_NATIVE_EXTENSIONS=native`.
...
- [x] **Workspace export semantics** — `ghostcrab_workspace_export_model` aligned with [docs/contracts/workspace-model-export.schema.json](docs/contracts/workspace-model-export.schema.json); examples under [docs/contracts/examples/](docs/contracts/examples/).
...
- [ ] **mindCLI** — verify batch / ingest paths set `workspace_id`, prefer **Layer 1** writes when DDL + `sync_spec` exist, and avoid duplicate or trigger-bypassing inserts into `mfo_facets` (confirm in the mindCLI repository; not vendored here).
...
Items explicitly **postponed after V1** in [.cursor/plans/ghostcrab_v1_reduced_plan_9f3c2b11.plan.md](.cursor/plans/ghostcrab_v1_reduced_plan_9f3c2b11.plan.md) (section *deferred after V1*), with matching detail in [.cursor/plans/ghostcrab_onboarding_rails_rewrite_5ad7bb1e.plan.md](.cursor/plans/ghostcrab_onboarding_rails_rewrite_5ad7bb1e.plan.md) (P1.3, P2.1–P2.5, P4.3). This section is a **backlog pointer only** — not a commitment order.
...
1. If V1 **intent or DoD** changes, update [docs/AUDIT_V1_TRACKING.md](docs/AUDIT_V1_TRACKING.md) and the [.cursor/plans/ghostcrab_v1_reduced_plan_9f3c2b11.plan.md](.cursor/plans/ghostcrab_v1_reduced_plan_9f3c2b11.plan.md) when you edit the formal plan.
