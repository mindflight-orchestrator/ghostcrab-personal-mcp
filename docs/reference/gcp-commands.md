...
| **Backup / restore** | `gcp brain backup …`, `gcp brain load …` | Export workspace, collection, or taxonomy backup bundles (includes `mindbrain_answer_artifacts` on full workspace export); restore `ghostcrab_backup_bundle` JSON. Restore refuses to replace an existing workspace unless both `--overwrite --confirm` are passed. `gcp brain export` is an alias for backup. |
