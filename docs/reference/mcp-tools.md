...
GhostCrab exposes 62 registered MCP tools: 13 recommended default tools and 49 extended tools. MCP `tools/list` returns the recommended default set; `gcp tools list`, `gcp tools verify`, and `ghostcrab_tool_search` expose discovery metadata for the full catalog.
