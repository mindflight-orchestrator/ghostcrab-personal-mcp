...
GhostCrab exposes 72 registered MCP tools, all returned by MCP `tools/list` and directly callable by name. A curated subset of 13 is flagged as recommended defaults (`title: GhostCrab recommended default`); the other 59 are extended tools. Use `gcp tools list`, `gcp tools verify`, and `ghostcrab_tool_search` for discovery metadata and domain filtering.
