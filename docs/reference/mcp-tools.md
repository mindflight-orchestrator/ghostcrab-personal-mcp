...
GhostCrab exposes 73 registered MCP tools, all returned by MCP `tools/list` and directly callable by name. A curated subset of 13 is flagged as recommended defaults (`title: GhostCrab recommended default`); the other 60 are extended tools. Use `gcp tools list`, `gcp tools verify`, and `ghostcrab_tool_search` for discovery metadata and domain filtering.
...
```json
{
  ...
  "surface_version": "2026-07-10",
  "generated_at": "2026-09-01T13:45:45.545Z"
}
```
...
| `include_agent_facts` | no | `boolean` | When true, run agent_facts FTS bootstrap (ensureFactsFtsSync) after collection reindex. |
| `scope` | no | `string` | all: collections loop + agent_facts FTS; collections: per-collection reindexAll only; graph: workspace graph reindex only. |
