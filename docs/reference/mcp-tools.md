...
GhostCrab exposes 74 registered MCP tools, all returned by MCP `tools/list` and directly callable by name. A curated subset of 13 is flagged as recommended defaults (`title: GhostCrab recommended default`); the other 61 are extended tools. Use `gcp tools list`, `gcp tools verify`, and `ghostcrab_tool_search` for discovery metadata and domain filtering.
...
| Subsystem | facets |
| Access | write |
| Required arguments | `workspace_id`, `accepted_by` |

| Argument | Required | Type | Description |
|----------|----------|------|-------------|
| `workspace_id` | yes | `string` | - |
| `proposal_id` | no | `string` | Accepted proposal id. A full proposal object is required for persistence in v1. |
| `proposal` | no | `object` | Learning proposal returned by ghostcrab_business_query_answer. |
| `accepted_by` | yes | `string` | - |
| `persist_to` | no | `array` | - |
...
| `target_kind` | no | `string` | Read original document or chunk assignments. Chunk assignments retain their exact chunk_index. |
| `doc_id` | no | `integer\|string` | Exact document id; use a decimal string for ids above JavaScript safe integer range. |
| `chunk_index` | no | `integer` | - |
| `ontology_id` | no | `string` | Exact taxonomy/ontology identity owning the assignment. |
...
| `collection_facet_value` | no | `string` | Explicit resolved collection facet value (substring). Independent of the natural-language query; discover the vocabulary first. Required to run the collection-facet fallback. |
...
| Subsystem | graph |
| Access | read |
| Required arguments | `assertion_ref` |

| Argument | Required | Type | Description |
|----------|----------|------|-------------|
| `workspace_id` | no | `string` | - |
| `assertion_ref` | yes | `object` | - |
| `include_text` | no | `boolean` | - |
| `limit` | no | `integer` | - |
| `cursor` | no | `string` | Opaque cursor returned by the previous evidence page. |
...
| Argument | Required | Type | Description |
|----------|----------|------|-------------|
| `workspace_id` | no | `string` | Workspace id used to resolve the default ontology and filter rules. Example: immeuble-demo. |
...
| `workspace_id` | no | `string` | Workspace scope for deletion. Defaults to MCP session workspace. |
...
| `workspace_id` | no | `string` | Workspace scope for the imported rules. Example: immeuble-demo. Defaults to MCP session workspace. |
...
| `validation_mode` | no | `string` | warn stores rules even when ontology references are missing; strict rejects unknown entity, relation, or target types. |
...
| Argument | Required | Type | Description |
|----------|----------|------|-------------|
| `workspace_id` | no | `string` | Workspace id to inspect. Defaults to the current MCP session workspace. |
| `ontology_id` | no | `string` | Optional ontology id. When omitted, MindBrain uses the workspace default ontology. |
| `limit` | no | `integer` | Maximum evaluation rows to return. |
...
| Argument | Required | Type | Description |
|----------|----------|------|-------------|
| `workspace_id` | no | `string` | Workspace id to evaluate. Defaults to the current MCP session workspace. |
| `ontology_id` | no | `string` | Optional ontology id. When omitted, MindBrain uses the workspace default ontology. |
| `limit` | no | `integer` | Maximum transition events to include in the response. |
| `create_remediation_actions` | no | `boolean` | When true, invalid→valid transitions may create proposed quality_remediation_action rows if the rule metadata declares remediation_action. |
...
| `workspace_id` | no | `string` | Workspace id to inspect. Defaults to the current MCP session workspace. |
| `ontology_id` | no | `string` | Optional ontology id. When omitted, MindBrain uses the workspace default ontology. |
| `limit` | no | `integer` | Maximum event rows to return. |
...
| Subsystem | pragma |
| Access | write |
| Required arguments | `slug`, `public_label`, `definition` |

| Argument | Required | Type | Description |
|----------|----------|------|-------------|
| `slug` | yes | `string` | Stable artifact slug; the id is live_answer_view__<slug>. |
| `public_label` | yes | `string` | User-facing label. |
| `definition` | yes | `object` | Non-empty declarative view definition. Top-level materialized is reserved. |
| `workspace_id` | no | `string` | Optional explicit override. When omitted, the active session workspace is used; the effective workspace is mandatory. |
...
| Argument | Required | Type | Description |
|----------|----------|------|-------------|
| `workspace_id` | no | `string` | Optional workspace id override. Defaults to the active MCP workspace. |
...
| Subsystem | ontology |
| Access | write |
| Required arguments | `ontology_id` |

| Argument | Required | Type | Description |
|----------|----------|------|-------------|
| `workspace_id` | no | `string` | Target workspace id. Defaults to the current MCP session workspace. |
| `ontology_id` | yes | `string` | Ontology id to project into the MCP schema registry. |
...
|----------|----------|------|-------------|
| `workspace_id` | no | `string` | Target workspace id. Defaults to the current MCP session workspace. |
| `ontology_id` | no | `string` | Ontology id to reconcile. When omitted, MindBrain resolves the workspace default ontology. |
| `limit` | no | `integer` | Maximum issue rows to return. |
...
| Argument | Required | Type | Description |
|----------|----------|------|-------------|
| `selection_mode` | no | `string` | Search plan content, or select one known plan by exact scope without filtering its definition by query. |
| `plan_id` | no | `string` | Optional exact projection row id in exact mode; scope and agent must also match. |
| `query` | yes | `string` | - |
| `agent_id` | no | `string` | - |
| `scope` | no | `string` | - |
| `limit` | no | `integer` | - |
...
|----------|----------|------|-------------|
| `workspace_id` | no | `string` | Target workspace id. Overrides session context for this call only. |
| `document_table_id` | no | `integer` | Optional facet table_id for graph entity-document projection when running graph-only fallback (no discoverable collections). |
| `include_agent_facts` | no | `boolean` | When true, reconcile the native facts index for this workspace after collection reindex. |
| `scope` | no | `string` | facts: native fact-index reconciliation for this workspace; all: collections loop + agent_facts FTS; collections: per-collection reindexAll; graph: workspace graph reindex. |
...
| `valid_from` | no | `string` | Optional start of validity in YYYY-MM-DD format. Defaults to today. Backdate it when importing a fact that was already true earlier; facts dated in the future stay out of reads until that date. |
...
| `mode` | no | `string` | - |
| `execution` | no | `string` | native_required requires the native index and never falls back to SQL keyword search. |
...
| `valid_from` | no | `string` | Optional start of validity in YYYY-MM-DD format, applied only when creating a new record. Ignored on an update: rewriting the start date of an existing record would falsify its history. |
