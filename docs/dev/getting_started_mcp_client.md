...
This guide shows the **maintainer / PostgreSQL** bootstrap path: local Docker, stdio MCP server, and a minimal client smoke test.
**If you installed from npm** ([`@mindflight/ghostcrab-personal-mcp`](../../README.md)), use [INSTALL.md](../../INSTALL.md) and **`gcp brain up`** instead — you do not need this Docker flow for the default SQLite SKU.
...
This builds the package, validates the tarball, starts the native Docker database with **facet, graph, and projection** PostgreSQL extensions (`pg_facets`, `pg_dgraph`, `pg_pragma`), applies migrations, runs the MCP smoke scenarios, and tears Docker down again.
...
For the detailed contract, see [mcp_tools_contract.md](../mcp_tools_contract.md). For Codex-specific setup, see [codex_integration.md](codex_integration.md).
