...
**Release candidate:** `@mindflight/ghostcrab-personal-mcp@0.7.1` · MindBrain backend **1.9.0** (commit `500f381`) · MCP tool surface **`2026-09-14`**
...
| **Projections / Artifacts (extended)**      | `ghostcrab_projection_get`, `ghostcrab_projections_list`, `ghostcrab_artifact_get`, `ghostcrab_live_create`, `ghostcrab_live_refresh`                                                                                                                                                                        |
...
MindBrain ships as a Git submodule at `vendor/mindbrain` (**1.9.0**, including exact scoped plan selection and the full agent-fact validity window on the native read paths — check `git submodule status` for the exact pin). Clone with:
