...
export async function cleanupTestDatabase(database: DatabaseClient): Promise<void> {
  if (database.kind === "sqlite") {
    ...
    await database.query("DELETE FROM mfo_projections");
    await database.query("DELETE FROM mfo_agent_state");
    await database.query("DELETE FROM mfo_facets");
  ...
  await database.query(
    database.kind === "sqlite"
      ? `
        ...
        `
      : `
          INSERT INTO mfo_facets (schema_id, content, facets, valid_until)
