...
const richProposal = JSON.parse(
  readFileSync(
    join(__dirname, "../../fixtures/crm-outbound/rich-proposal.json"),
    "utf8"
  )
...
const goldenExport = JSON.parse(
  readFileSync(
    join(__dirname, "../../fixtures/crm-outbound/golden-export.json"),
    "utf8"
  )
) as ExportPayload;
...
describe.sequential(
  "CRM outbound golden-path — rich semantics end-to-end",
  () => {
    afterAll(async () => {
      try {
    ...
    it("creates crm workspace and proposes DDL with rich semantics", async () => {
      ...
      const createResult = await workspaceCreateTool.handler(
        {
          ...
          label: "CRM Outbound",
          description:
            "Mini CRM outbound: accounts, contacts, campaigns, enrollments, and activities.",
      ...
      const migrationId = (
        proposeResult.structuredContent as Record<string, unknown>
      ...
      const semanticsApplied = ((
        execResult.structuredContent as Record<string, unknown>
    ...
    it("exports a rich model for the mini outbound CRM", async () => {
      ...
      const campaigns = tables.find(
        (table) => table.table_name === "campaigns"
      );
      const enrollments = tables.find(
        (table) => table.table_name === "campaign_enrollments"
      );
      const activities = tables.find(
        (table) => table.table_name === "activities"
      );
      ...
      const campaignStatus = columns.find(
        (column) =>
          column.table_name === "campaigns" && column.column_name === "status"
      );
      const activityType = columns.find(
        (column) =>
          column.table_name === "activities" &&
          column.column_name === "activity_type"
      );
      const activityOutcome = columns.find(
        (column) =>
          column.table_name === "activities" && column.column_name === "outcome"
      );
      ...
      const worksAt = relations.find(
        (relation) =>
          relation.source_table === "contacts" &&
          relation.target_table === "accounts"
      );
      ...
      const hints = (exportData.generation_hints ?? {}) as Record<
        string,
        unknown
      >;
      ...
      expect(
        tableOrder.findIndex((table) => table.includes("accounts"))
      ).toBeLessThan(
