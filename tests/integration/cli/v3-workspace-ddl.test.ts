 ...
 *   migration 011 applied → mfo_facets.source_ref column exists
...
describe.sequential("V3 Plan B integration — workspace + DDL lifecycle", () => {
  ...
  it("mfo_facets.workspace_id column exists", async () => {
    const rows = await harness.database.query<{ name: string }>(
      `PRAGMA table_info(mfo_facets)`
    );
    expect(rows.some((row) => row.name === "workspace_id")).toBe(true);
  });

  it("mfo_facets.source_ref column exists", async () => {
    const rows = await harness.database.query<{ name: string }>(
      `PRAGMA table_info(mfo_facets)`
    );
  ...
  it("unique index on (source_ref, workspace_id) exists", async () => {
    const rows = await harness.database.query<{ name: string }>(
      `SELECT name FROM sqlite_master
       WHERE type = 'index' AND name = 'idx_mfo_facets_source_ref_workspace'`
    );
    expect(rows).toHaveLength(1);
  });
  ...
  it("source_ref contract: two historical rows (source_ref=NULL) are allowed", async () => {
    await harness.database.query(
      `INSERT INTO mfo_facets (schema_id, content, facets, workspace_id, source_ref)
       VALUES ($1, 'hist1', '{}', 'default', NULL),
              ($1, 'hist2', '{}', 'default', NULL)`,
      [sourceRefSchemaId]
    );
    const rows = await harness.database.query<{ content: string }>(
      `SELECT content FROM mfo_facets WHERE schema_id = $1 ORDER BY content`,
      [sourceRefSchemaId]
    );
    expect(rows.length).toBeGreaterThanOrEqual(2);
    await harness.database.query(`DELETE FROM mfo_facets WHERE schema_id = $1`, [
      sourceRefSchemaId
    ]);
  });
  ...
  it("source_ref contract: same source_ref on different workspaces is allowed", async () => {
    ...
    const rows = await harness.database.query<{ source_ref: string }>(
      `SELECT source_ref FROM mfo_facets WHERE schema_id = $1 AND source_ref = $2`,
      [sourceRefSchemaId, refCross]
    );
    expect(rows).toHaveLength(2);
    await harness.database.query(
      `DELETE FROM mfo_facets WHERE schema_id = $1`,
      [sourceRefSchemaId]
    );
    await harness.database.query(
