...
describe.sequential(
  "ghostcrab_projections_list (integration, no mocks)",
  () => {
    ...
    beforeEach(async ({ skip }) => {
      ...
      if (!backendReachable) {
        skip(
          "Integration backend unavailable; skipping projections-list tests."
        );
        return;
      }
    ...
    it("executes graph_entity SQL with metadata_json collection_id", async () => {
      ...
      expect(
        rows.find((row) => row.projection_id === graphProjectionId)
          ?.collection_id
      ).toBe(`${WS_ID}::docs`);
    });
    ...
    it("lists registry rows and graph projection ids with routing hints", async () => {
      ...
      const live = projections.find(
        (row) => row.artifact_kind === "live_answer_view"
      );
      const plan = projections.find(
        (row) => row.artifact_kind === "analysis_plan"
      );
