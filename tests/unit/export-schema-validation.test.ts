...
function validateExport(
  ...
  if (!Array.isArray(data.tables)) {
    e("tables", "must be array");
  } else {
    for (const [i, t] of (data.tables as Record<string, unknown>[]).entries()) {
      const tp = `tables[${i}]`;
      if (typeof t.schema_name !== "string")
        e(`${tp}.schema_name`, "must be string");
      if (typeof t.table_name !== "string")
        e(`${tp}.table_name`, "must be string");
      if (
        t.table_role !== null &&
        t.table_role !== undefined &&
        !TABLE_ROLES.has(t.table_role as string)
      ) {
        e(`${tp}.table_role`, `invalid value: ${String(t.table_role)}`);
      }
      if (
        t.generation_strategy !== null &&
        t.generation_strategy !== undefined &&
        !GENERATION_STRATEGIES.has(t.generation_strategy as string)
      ) {
        e(
          `${tp}.generation_strategy`,
          `invalid value: ${String(t.generation_strategy)}`
        );
      }
      if (
        t.volume_driver !== null &&
        t.volume_driver !== undefined &&
        !VOLUME_DRIVERS.has(t.volume_driver as string)
      ) {
        e(`${tp}.volume_driver`, `invalid value: ${String(t.volume_driver)}`);
      }
      if (typeof t.emit_facets !== "boolean")
        e(`${tp}.emit_facets`, "must be boolean");
      if (typeof t.emit_graph_entities !== "boolean")
        e(`${tp}.emit_graph_entities`, "must be boolean");
      if (typeof t.emit_graph_relations !== "boolean")
        e(`${tp}.emit_graph_relations`, "must be boolean");
      if (typeof t.emit_projections !== "boolean")
        e(`${tp}.emit_projections`, "must be boolean");
    }
  }
  ...
  if (data.columns !== undefined) {
    if (!Array.isArray(data.columns)) {
      e("columns", "must be array");
    } else {
      for (const [i, c] of (
        data.columns as Record<string, unknown>[]
      ).entries()) {
        const cp = `columns[${i}]`;
        if (typeof c.schema_name !== "string")
          e(`${cp}.schema_name`, "must be string");
        if (typeof c.table_name !== "string")
          e(`${cp}.table_name`, "must be string");
        if (typeof c.column_name !== "string")
          e(`${cp}.column_name`, "must be string");
        if (
          c.column_role !== null &&
          c.column_role !== undefined &&
          !COLUMN_ROLES.has(c.column_role as string)
        ) {
          e(`${cp}.column_role`, `invalid value: ${String(c.column_role)}`);
        }
        if (
          c.semantic_type !== null &&
          c.semantic_type !== undefined &&
  ...
  if (data.relations !== undefined) {
    if (!Array.isArray(data.relations)) {
      e("relations", "must be array");
    } else {
      for (const [i, r] of (
        data.relations as Record<string, unknown>[]
      ).entries()) {
        const rp = `relations[${i}]`;
        if (typeof r.source_schema !== "string")
          e(`${rp}.source_schema`, "must be string");
        if (typeof r.source_table !== "string")
          e(`${rp}.source_table`, "must be string");
        if (typeof r.source_column !== "string")
          e(`${rp}.source_column`, "must be string");
        if (typeof r.target_schema !== "string")
          e(`${rp}.target_schema`, "must be string");
        if (typeof r.target_table !== "string")
          e(`${rp}.target_table`, "must be string");
        if (typeof r.target_column !== "string")
          e(`${rp}.target_column`, "must be string");
        if (
          r.relation_role !== null &&
          r.relation_role !== undefined &&
        ...
        if (
          r.hierarchical !== undefined &&
          typeof r.hierarchical !== "boolean"
        ) {
...
describe("workspace-model-export schema validation", () => {
  ...
  it("kanban fixture has non-null entity_family on all tables", () => {
    ...
    for (const table of tables) {
      expect(
        table.entity_family,
        `${table.table_name}.entity_family`
      ).not.toBeNull();
    }
  });
  ...
  it("JSON schema seed_multiplier defaults are correct per spec", () => {
    const schema = JSON.parse(
      readFileSync(
        join(contractsDir, "workspace-model-export.schema.json"),
        "utf8"
      )
    ) as Record<string, unknown>;
