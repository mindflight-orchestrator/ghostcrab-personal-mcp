...
describe("mcp-tool-policy", () => {
  ...
  it("basic preset exposes exactly 14 allow rules", () => {
    const policy = buildToolPermissionPreset("basic", { serverName: server });
    expect(policy.allow).toHaveLength(14);
    expect(policy.ask).toHaveLength(0);
    expect(getBasicToolNames()).toHaveLength(14);
