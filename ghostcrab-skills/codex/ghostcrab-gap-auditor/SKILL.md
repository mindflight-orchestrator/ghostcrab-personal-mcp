...
References: [glossary](../../../docs/explanation/glossary.md), [operator catalog](../../../docs/reference/operator-catalog.md).

## Gap categories

- `no_projection`: no matching Type A scope for the question.
- `projection_contract_only`: Type A exists in `projections` but no supporting facts or graph rows surfaced.
...
- `missing_snapshot`: no Type B `ProjectionResult` via `ghostcrab_projection_get`.
...
5. `ghostcrab_pack` or `ghostcrab_projection_get` when projections matter.
...
Each gap entry: `category`, `severity` (`low|medium|high`), `evidence`, `impact`.
...
- Do not label a projection ready if only the Type A contract exists and the user asked for live operational facts.
- Type B is optional; state when Type A + graph search is enough.
