...
References: [RUNTIME_QUERY_PIPELINE.md](../../shared/RUNTIME_QUERY_PIPELINE.md), [ARTIFACT_KINDS.md](../../shared/ARTIFACT_KINDS.md), [PROJECTIONS_DISCOVERY.md](../../shared/PROJECTIONS_DISCOVERY.md), [ENUM_BUSINESS_FACETS.md](../../shared/ENUM_BUSINESS_FACETS.md).

## Delivery context (optional)

Resolve `{starterkit}` via [STARTERKIT_PATHS.md](../../shared/STARTERKIT_PATHS.md) — Phase B runtime section in `{starterkit}/personal-mcp/SOP_SEQUENCE.md`.

## Workflow

1. `ghostcrab_status` + active workspace.
2. `ghostcrab_projections_list` when projection ids or scopes are unknown.
3. When the question filters on a domain enum (status, type, tier): `ghostcrab_facet_catalog` / `ghostcrab_facet_inspect` with `<module>.<slot_snake_case>` keys — see [ENUM_BUSINESS_FACETS.md](../../shared/ENUM_BUSINESS_FACETS.md).
