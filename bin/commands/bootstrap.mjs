...
`.trim()
  ...
  if (existsSync(envDest)) {
    ...
    warn(
      `[ghostcrab] bootstrap: .env.example not found in package at ${envSrc}`
    );
  } else {
    try {
      ...
      warn(
        `[ghostcrab] bootstrap: .env         ERROR — ${e instanceof Error ? e.message : e}`
      );
    }
  }
  ...
  if (existsSync(dataDir)) {
    ...
    try {
      ...
      warn(
        `[ghostcrab] bootstrap: data/        ERROR — ${e instanceof Error ? e.message : e}`
      );
    }
  }
  ...
  for (const { name, optional } of DOC_SYMLINKS) {
    ...
    if (!existsSync(src)) {
      if (!optional)
        warn(`[ghostcrab] bootstrap: ${name}: not found in package — skipped`);
      continue;
    }
    ...
    if (existsSync(dest)) {
      ...
      if (st?.isSymbolicLink()) {
        try {
          ...
          log(
            `[ghostcrab] bootstrap: ${name.padEnd(28)} refreshed → ${target}`
          );
        } catch (e) {
          warn(
            `[ghostcrab] bootstrap: ${name}: refresh ERROR — ${e instanceof Error ? e.message : e}`
          );
        }
      } else {
        log(
          `[ghostcrab] bootstrap: ${name.padEnd(28)} skipped (real file exists — not overwritten)`
        );
      }
    } else {
      try {
        ...
        warn(
          `[ghostcrab] bootstrap: ${name}: ERROR — ${e instanceof Error ? e.message : e}`
        );
      }
    }
  }
