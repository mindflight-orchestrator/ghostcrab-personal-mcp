...
5. **Confirmed architecture regression: vector work had returned to
   JavaScript.** Search decoded stored vectors and recomputed cosine/hybrid
   scores, guidance embedded and compared its aliases on every call, and
   backfill mirrored rows one by one. Search and batch persistence now delegate
   to MindBrain Zig. A repository-wide source guard rejects JavaScript vector
   decoding/scoring, and a disposable-backend test exercises the real batch
   boundary.
...
| `ghostcrab_business_query_register`                                                                                | Proposal fingerprint, activation state, fact persistence and native vector indexing                                         | **Mock:** `tests/tools/business-query-register-embeddings.test.ts` verifies the numeric vector reaches the native writer; **Source:** `src/tools/business-query-learning/register-proposal.ts` | Idempotency races and final read-back scoped to the stored workspace column                                                                                 |
...
| Validation                                                               | Result                                                          |
| ------------------------------------------------------------------------ | --------------------------------------------------------------- |
| Default suite, with global and critical-module coverage gates            | 868 passed, 20 skipped                                          |
| Required canonical SQLite integrity suite                                | 52 passed, zero skipped                                         |
| Required native integrity suite                                          | 15 passed, zero skipped                                         |
...
| Global TypeScript coverage                                               | 68.16% lines, 56.06% branches                                   |
...
The 20 default-suite skips are not represented as passes. Twelve of those cases
(memory, collection lifecycle and the embedding batch boundary) run explicitly
in the separate native gate, alongside the three MCP scenarios. This does not
certify every integration test or every supported platform.
